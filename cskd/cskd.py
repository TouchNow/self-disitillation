...
class CSKDLoss(torch.nn.Module):
    ...
    def align_stu_logits(self, stu_dense_logits):
        N, M, C = stu_dense_logits.shape  # N 196 C
    ...
    def get_loss_cskd(
        ...
        teacher_logits = decay_ratio * teacher_dense_logits + (
            1 - decay_ratio
        ) * teacher_global_logits.reshape(N, C, 1, 1)
        # cskd loss
        if self.cfg.cskd_loss_type == "hard":
            loss_cskd = F.cross_entropy(stu_dense_logits, teacher_logits.argmax(dim=1))
